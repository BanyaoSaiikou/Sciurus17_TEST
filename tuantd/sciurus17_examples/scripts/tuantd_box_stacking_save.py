    ...
    while True:
		...
		if is_next_box_detected == False:
			...
			string_data = data.tostring()
                        guai=str.encode(str(len(string_data)).ljust(16))
                        print("!!!!!!!",guai)
                        #print("!!!!!!!type",type(guai))#string
			...
			if data[1][current_box_idx] != []:
				...
				if is_move_needed:
					...
					string_data = data_2.tostring()
                                        ...
                                        guai2=str.encode(str(len(string_data)).ljust(16))
                                        print("guai2",guai2)
					#conn.send(pickle.dumps([1]))
					conn.send(pickle.dumps([1]))
					conn.send(guai2);
...
def main():
	...
	while not rospy.is_shutdown():
		...
		get_object_positions(r)
		data=pickle.dumps([1, last_box_positions])
                #print("pick up data1!!!!",data)
		conn.send(data)
                time.sleep(0.5)#shan 
		data = pickle.loads(conn.recv(BUFFER_SIZE))
                print("pick up data2!!!!",data)
		if not data:
			print("connection closed")
			return
		
		if data[0] == 4:#if data[0] == 1:
			while not rospy.is_shutdown():
